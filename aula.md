# Passo a Passo — Rede Cisco: Filial Rondônia x Filial São Paulo
...
```
        Filial RONDÔNIA (RO)                              Filial SÃO PAULO (SP)
   +------------------------+                        +------------------------+
   |   VLAN 10 - Vendas     |                         |   VLAN 30 - Vendas     |
   |   192.168.10.0/24      |                         |   192.168.30.0/24      |
   |                        |                         |                        |
   |   VLAN 20 - Adm        |                         |   VLAN 40 - Adm        |
   |   192.168.20.0/24      |                         |   192.168.40.0/24      |
   +-----------+------------+                         +-----------+------------+
               |  Gi0/1 (trunk 802.1Q)                            |  Gi0/1 (trunk 802.1Q)
          [SW-RO]                                              [SW-SP]
               |  Gi0/0                                             |  Gi0/0
          [R-RO]  Se0/3/0 ------------ WAN (Serial) ------------ Se0/3/0  [R-SP]
...
- **Link entre filiais:** Serial ponto-a-ponto (192.168.100.0/30).

## 2. Equipamentos usados

| Papel | Modelo | Detalhe |
|-------|--------|---------|
| Roteador (2x) | **Cisco 2901** | + módulo **HWIC-2T** encaixado no **slot 3** → gera as interfaces `Serial0/3/0` e `Serial0/3/1` |
| Switch (2x) | **Cisco 2960-24TT-L** | 24 portas FastEthernet (Fa0/1–0/24) + 2 portas GigabitEthernet (Gi0/1, Gi0/2) |

> ⚠️ O slot do módulo HWIC-2T pode variar conforme onde você encaixar no Packet
